...
from djimaging.utils.image_utils import rotate_image, resize_image, rescale_image

try:
    import cv2
except ImportError as e:
    warnings.warn(f'Failed to import cv2 with error: {e}')
    cv2 = None

from djimaging.user.alpha.tables.morphology import morphology_roi_utils
...
def match_rec_to_stack(
        ...
        blur_decay=5., blur_power=2., blur_factor=0.9, blur_postpower=0.5, pad_linestack=0,
    ...
    crop_xy0, crop_xyz = get_crop(
        ...
        pad_linestack=pad_linestack)

    fit_dict = find_best_match(
        stack=crop_xyz, template=rec_rot, rois_xyz=roi_coords_rot, rescales=rescales,
...
def rotate_rec(ch_average, wparams_rec, wparams_stack, setup_xscale, angle_adjust=0):
    ...
    rec_rot = rotate_image(rec_resized, ang_deg, order=1)
    return rec_rot
...
def rotate_roi(roi_mask, wparams_rec, wparams_stack, setup_xscale, angle_adjust=0, no_rois_lost=True):
    ...
    rec_rois_rot = rotate_image(rec_rois, ang_deg, cval=1, order=0)
    if no_rois_lost:
        rec_rois_rot = add_back_lost_rois(rec_rois=rec_rois, mod_rois=rec_rois_rot)
    ...
    xn, yn = rotate_point_around_center(px, py, ang_deg, cx, cy, shift_x, shift_y)

    return roi_ids, rec_rois_rot, np.vstack([xn, yn]).T


def rotate_point_around_center(px, py, ang_deg, cx=0, cy=0, shift_x=0, shift_y=0):
...
def get_crop(linestack, rec_shape, rec_cxy, pixel_sizes_stack, pad_scale=1., pad_more=50, shift=(0, 0),
    ...
    if pad_linestack > 0:
        linestack = np.pad(linestack.copy(), (pad_linestack, pad_linestack), mode='constant', constant_values=0)

    crop_x0, crop_y0, crop_x1, crop_y1 = get_crop_idxs(linestack, rec_cxy[0], rec_cxy[1], sizex, sizey, shift)
    crop_xy0 = np.array([crop_x0 - pad_linestack, crop_y0 - pad_linestack])
...
def resize_rec(ch_average, wparams_rec, wparams_num, setup_xscale):
    ...
    return resize_image(ch_average, output_shape=output_shape, order=0)


def create_template_from_recording(rec, npixartifact, data_stack_name):
    data = None
    if 'match_template' in rec:
        data = rec['match_template']
...
def resize_roi(roi_mask, wparams_rec, wparams_stack, setup_xscale, no_rois_lost=True):
    """Rescale ROIs"""
    output = np.round(rescale_image(
        image=roi_mask, scale=get_scale_factor_xy(wparams_rec, wparams_stack, setup_xscale)))
    if no_rois_lost:
        output = add_back_lost_rois(rec_rois=roi_mask, mod_rois=output)

    return output


def add_back_lost_rois(rec_rois, mod_rois):
...
def blur_stack(stack, blur, postblurpower=None, inplace=False):
    assert np.min(stack) == 0
    assert np.max(stack) == 1
    if not inplace:
        stack = stack.copy()

    max_unblurred = np.max(stack)
    stack = gaussian_filter(stack.astype('float32'), sigma=blur)
    stack /= np.max(stack)  # Renormalize
    if postblurpower is not None:
        stack = stack ** postblurpower
    stack *= max_unblurred
    return stack


def compute_weighted_z_proj(zidx, stack, decay=5., power=2., plot=False):
    assert stack.ndim == 3
    assert 0 <= zidx <= stack.shape[2] - 1

    zdists = np.concatenate([np.arange(zidx, 0, -1), np.arange(0, stack.shape[2] - zidx)])
    zweights = np.exp(-np.abs(zdists / decay) ** power)
    weighted_z_proj = np.max(stack * zweights, axis=2)

    if plot:
        plot_weighted_z_proj(zdists=zdists, zweights=zweights, z_slice=stack[:, :, zidx], z_proj=weighted_z_proj)

    return weighted_z_proj


def plot_weighted_z_proj(zdists, zweights, z_slice, z_proj):
    fig, axs = plt.subplots(1, 4, figsize=(12, 5))
    axs[0].plot(zdists)
    axs[1].plot(zweights)
    im = axs[2].imshow(z_slice, cmap='Greys')
    plt.colorbar(im, ax=axs[2])
    im = axs[3].imshow(z_proj)
...
def find_best_match(
        stack, template, exp_center=None, decay=5., power=2., blur=1., postblurpower=0.5,
        rescales=None, threshold=True, dist_score_factor=1e-3, max_factor=0.75,
        rois_xyz=None, plot_zidxs=False, plot_steps=False, plot_result=True, savefilename=None):
    """Find best match between template and 3d stack.
    :param stack: xyz stack
    :param template: template to match to stack
    :param exp_center: expected center of template in stack coordinates
    :param decay: decay of weights for stack z-projection
    :param power: power of weights for stack z-projection
    :param blur: blur of stack before matching
    :param postblurpower: power after blurring z-stack
    :param rescales: list of rescales to try
    :param threshold: threshold for template preprocessing
    :param dist_score_factor: factor for distance score, set to zero to disable distance
    :param max_factor: multiply template by this factor before matching
    :param rois_xyz: coordinates of ROIs in rotated stack
    :param plot_zidxs: plot zidxs considered
    :param plot_steps: plot intermediate steps
    :param plot_result: plot final result
    :param savefilename: save plot to this filename
    """

    stack_layer_sums = np.sum(stack, axis=(0, 1))
    zidxs = np.where(stack_layer_sums > np.median(stack_layer_sums))[0]

    if plot_zidxs:
        fig, ax = plt.subplots(1, 1, figsize=(5, 3))
        ax.plot(stack_layer_sums)
    ...
    if blur is not None:
        stack = blur_stack(stack, blur=blur, postblurpower=postblurpower, inplace=False)

    if rescales is None:
        rescales = [1.0]

    for rescale in np.unique(rescales):
        template_raw = rescale_image(template.copy(), rescale, order=0).T.astype('float32')
        ...
        for zidx in zidxs:
            ...
            weighted_z_proj = compute_weighted_z_proj(
                zidx=zidx, stack=stack, decay=decay, power=power, plot=False).T

            score, pos, score_map = match_template_to_image(
                image=weighted_z_proj, template=template_fit,
                exp_center=exp_center, dist_score_factor=dist_score_factor)

            if plot_steps:
                plot_match_template_to_image(
                    image=weighted_z_proj, template_fit=template_fit,
                    score_map=score_map, best_xy=pos, best_score=score,
                    template_raw=template_raw, roi_coords_rot=rois_xyz,
                    exp_center=exp_center, savefilename=None)

            if best_score is None or score > best_score:
                best_score = score
                best_score_map = score_map
                best_weighted_z_proj = weighted_z_proj
    ...
    if plot_result:
        plot_match_template_to_image(
            image=best_weighted_z_proj, template_fit=best_template_fit,
            score_map=best_score_map, best_xy=best_xyz[:2], best_score=best_score,
            template_raw=best_template_raw, roi_coords_rot=rois_xyz,
            exp_center=exp_center, savefilename=savefilename)

    best_fit_dict = dict(
        score=best_score,
        score_map=np.asarray(best_score_map).astype(np.float32),
        linestack_xy=np.asarray(best_weighted_z_proj).astype(np.float32),
