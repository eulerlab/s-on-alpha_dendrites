...
def split_trace_by_reps(trace, times, triggertimes, ntrigger_rep, delay=0., atol=0.128, allow_drop_last=True,
                        pad_trace=False, dt_base_rng=None):
    ...
    if dt_base_rng is None:
        return snippets, snippets_times, snippets_triggertimes, droppedlastrep_flag

    dt_base_before, dt_base_after = dt_base_rng
    baselines = np.zeros(len(t_idxs))

    t_base_idxs_1 = [find_closest(target=tt + dt_base_before, data=times, atol=atol, as_index=True)
                     for tt in triggertimes[::ntrigger_rep]]
    t_base_idxs_2 = [find_closest(target=tt + dt_base_after, data=times, atol=atol, as_index=True)
                     for tt in triggertimes[::ntrigger_rep]]

    for i, (idx1, idx2) in enumerate(zip(t_base_idxs_1, t_base_idxs_2)):
        baselines[i] = np.mean(trace[idx1:idx2])

    snippets -= baselines

    return snippets, snippets_times, snippets_triggertimes, droppedlastrep_flag, baselines
