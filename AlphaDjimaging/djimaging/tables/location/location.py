...
class RelativeFieldLocationTemplate(dj.Computed):
    ...
    def make(self, key):
        ...
        if self.presentation_table is not None:
            pres_absx, pres_absy, pres_absz = (self.presentation_table().ScanInfo & key).fetch(
                'xcoord_um', 'ycoord_um', 'zcoord_um')
...
class RetinalFieldLocationWingTemplate(dj.Computed):
    database = ""
    ...
    def make(self, key):
        ...
        def loc_fun(x, y):
            if (x ** 2 + y ** 2) ** 0.5 < self._center_dist:
                return 'c'  # Center
            elif x > y:
                if -x < y:
                    return 'n'  # right
        ...
        wing_side = loc_fun(x=temporal_nasal, y=ventral_dorsal)

        rfl_key = key.copy()
        rfl_key['wing_side'] = wing_side
        self.insert1(rfl_key)
    ...
    def plot(self, restriction=None):
        ...
        sns.scatterplot(
            ax=ax, data=df_plot, x=self._temporal_nasal_key, y=self._ventral_dorsal_key, hue='wing_side')
