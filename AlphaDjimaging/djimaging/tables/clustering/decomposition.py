...
class FeaturesTemplate(dj.Computed):
    ...
    @property
    @abstractmethod
    def roikind_table(self):
        pass

    @property
    @abstractmethod
    def roicalibratedstackpos_table(self):
        pass
    ...
    def _roi_key_source(self, key):
        return (self.roi_table & key &
                (self.roikind_table & "roi_kind='roi'") &
                (self.roicalibratedstackpos_table() & "success_cal_flag=1")).proj()

    def fetch_traces(self, key, rtol=0.15, roi_filter_restr=None):
        ...
        tab = self._roi_key_source(key)

        restr = self._restr_filter if self._restr_filter is not None else dict()
        roi_filter_restr = roi_filter_restr if roi_filter_restr is not None else dict()

        if self.roi_filter_table is not None:
            tab = tab & (self.roi_filter_table & restr & roi_filter_restr)
    ...
    def make(self, key, verboselvl=1):
        ...
        roi_filter_restr = params_dict.pop('roi_filter_restr', None)

        ncomps = [int(ncomps_i) for ncomps_i in ncomps.split('_')] if len(ncomps) > 0 else []
        traces, times, roi_keys, stim_names = self.fetch_traces(key=key, roi_filter_restr=roi_filter_restr)

        if verboselvl > 1:
            print(f"Computing features for {key!r} with {len(traces[0])} traces.")

        features, traces_reconstructed, infos = compute_features(
            traces=traces, ncomps=ncomps, kind=kind, params_dict=params_dict,
            pre_standardize=False, post_standardize=kind in ['pca', 'sparse_pca'], verboselvl=verboselvl)
