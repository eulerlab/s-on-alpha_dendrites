...
class ChirpSurroundIndexTemplate(dj.Computed):
    database = ""
    ...
    def l_snippets_table(self):
        secondary_keys = get_secondary_keys(self.snippets_table)
        return (self.snippets_table & dict(stim_name=self._l_name)).proj(*secondary_keys, lChirp='stim_name')

    def g_snippets_table(self):
        secondary_keys = get_secondary_keys(self.snippets_table)
        return (self.snippets_table & dict(stim_name=self._g_name)).proj(*secondary_keys, gChirp='stim_name')

    @property
    def key_source(self):
        try:
            return self.l_snippets_table().proj() * self.g_snippets_table().proj()
        except (AttributeError, TypeError):
            pass

    def compute_entry(self, key, plot=False):
        polarity = self._fixed_polarity
        if polarity is None:
            raise NotImplementedError("Automatic polarity detection not implemented yet")
        elif polarity not in [-1, 1]:
            raise ValueError("Invalid polarity value")
        elif polarity == -1:
            raise NotImplementedError("Polarity -1 not implemented yet")

        l_snippets, l_snippets_times, l_triggertimes_snippets = (self.l_snippets_table() & key).fetch1(
            'snippets', 'snippets_times', 'triggertimes_snippets')
        g_snippets, g_snippets_times, g_triggertimes_snippets = (self.g_snippets_table() & key).fetch1(
            'snippets', 'snippets_times', 'triggertimes_snippets')

        dt = get_mean_dt(l_snippets_times.T)[0]
        if not np.isclose(get_mean_dt(g_snippets_times.T)[0], dt):
