...
class FieldWithConditionTemplate(FieldTemplate):
    @property
    def definition(self):
        new_line = 'cond1    :varchar(16)    # cond1 (pharmacological or other)\n        '
        d = super().definition
        i_primary = d.find('---')
        assert i_primary > 0
    ...
    def load_new_keys(self, key, field, pre_data_path, files, mask_alias, highres_alias, setupid, verboselvl):
        ...
        cond1 = get_condition(data_file=field_key["fromfile"], loc=condition_loc)
        for file in files:
            if get_condition(data_file=file, loc=condition_loc) != cond1:
                ValueError(f"{get_condition(data_file=file, loc=condition_loc)} != {cond1}, {files}")

        field_key["cond1"] = cond1
        roimask_key["cond1"] = cond1
        for avg_key in avg_keys:
            avg_key["cond1"] = cond1
    ...
    def add_experiment_fields(self, key, only_new: bool, verboselvl: int, suppress_errors: bool):
        ...
        for field, info in field2info.items():
            ...
            for data_file in info['files']:
                cond1 = get_condition(data_file, loc=condition_loc)
        ...
        for field, info in field2info.items():
            ...
            for cond1 in np.unique(conditions):
                condition_files = files[conditions == cond1]
                exists = len((self & key & dict(field=field, cond1=cond1)).fetch()) > 0
                if only_new and exists:
                    if verboselvl > 1:
                        print(f"\tSkipping field '{field}' '{cond1}' with files: {condition_files}")
                    continue

                if verboselvl > 0:
                    print(f"\tAdding field: '{field}' '{cond1}' with files: {condition_files}")
...
def scan_fields_and_files(pre_data_path: str, user_dict: dict, verbose: bool = False) -> dict:
    ...
    for file in sorted(os.listdir(pre_data_path)):
        ...
        datatype, animal, region, field, stimulus, cond1 = get_filename_info(file, **loc_mapper)
