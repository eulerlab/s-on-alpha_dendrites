...
class PreprocessTracesTemplate(dj.Computed):
    ...
    def plot1(self, key=None, xlim=None, ylim=None):
        ...
        ax = axs[0]
        plot_trace_and_trigger(time=trace_times, trace=trace - np.nanmean(smoothed_trace),
                               triggertimes=triggertimes, ax=ax, title=str(key))
        ax.set(ylabel='mean subtracted\nraw trace')

        ax = axs[1]
        plot_trace_and_trigger(time=trace_times, trace=smoothed_trace - np.nanmean(smoothed_trace),
