...
class PresentationTemplate(dj.Computed):
    ...
    def definition(self):
        ...
        -> self.stimulus_table
        -> self.params_table
        cond1             :varchar(16)     # cond1 (pharmacological or other)
        ---
        h5_header             :varchar(191)     # path to h5 file
    ...
    def add_field_presentations(self, key, only_new: bool, verboselvl: int, suppress_errors: bool):
        ...
        if verboselvl > 3:
            print(f"Found {len(field_files)} files in folder {pre_data_path} for field {field}:"
                  f"\n{field_files}")
        ...
        for data_file in field_files:
            ...
            if (stim == 'nostim') or (len(stim) == 0) or (stim.lower() not in stim_alias):
                continue
            if ("cond1" in key) and (key['cond1'] != cond1):
                continue
            pres_files.append(data_file)

        for data_file in pres_files:
            new_key = deepcopy(key)
            new_key["cond1"] = get_condition(data_file, loc=condition_loc)
