...
class DNoiseTraceTemplate(dj.Computed):
    ...
    def make(self, key):
        ...
        stim_idxs = np.arange(stim.shape[0], dtype="uint16")

        stim_idxs, trace, dt, t0, dt_rel_error = prepare_noise_data(
            trace=trace, tracetime=tracetime, stim=stim_idxs, triggertimes=stimtime,
        ...
        if stim_idxs.shape[0] != trace.shape[0]:
            raise ValueError(f"Stimulus and trace have different number of samples: "
                             f"{stim_idxs.shape[0]} vs. {trace.shape[0]}")
    ...
    def plot1(self, key=None, xlim=None):
        ...
        time, trace, stim_or_idxs = (self & key).fetch1('time', 'trace', 'stim')

        if stim_or_idxs.ndim == 1:  # This is in index space
            stim_original = (self.stimulus_table() & key).fetch1('stim_trace')
