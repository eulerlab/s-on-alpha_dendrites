...
def load_high_res_stack(pre_data_path, raw_data_path, highres_alias,
                        field, field_loc, cond1=None, condition_loc=None, allow_raw=True):
    ...
    filepath = scan_for_highres_filepath(
        ...
        cond1=cond1, condition_loc=condition_loc)
    ...
    if allow_raw:
        ...
        if filepath is not None:
            try:
                ...
                ch_stacks = {'wDataCh0': ch0_stack, 'wDataCh1': ch1_stack}
                return filepath, ch_stacks, wparams
            except OSError:
                warnings.warn(f'OSError when reading file: {filepath}')
                pass
...
def scan_for_highres_filepath(folder, field, field_loc, highres_alias, cond1=None, condition_loc=None, ftype='h5'):
    ...
    for file in field_files:
        ...
        is_condition = True if cond1 is None else \
            match_file(file, pattern=cond1, pattern_loc=condition_loc, ftype=ftype)
